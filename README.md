...
### Step 2: Transverse Beam Profile Scan
The lab beam has some finite transverse (x–y) spot size at the tile face that isn't precisely known. For each tile:
1. Run the position scan (Week 5 framework) using a series of candidate beam spot sizes (e.g., a Gaussian of varying sigma), in addition to the realistic energy spectrum.
2. Compare each resulting simulated light-yield-vs-position (uniformity) curve to the corresponding lab data curve.
3. Identify the spot size that gives the most consistent match to data (e.g., minimize chi-square between sim and data curve shapes).
...
(Extend to other placements — corner, edge, multiple SiPMs — as time allows, following the Week 12 optimization methodology.) Report predicted uniformity and total light yield per configuration and identify the placement expected to give the best combination of yield and uniformity.
