...
OUTPUT_DIR="test/OpNovice2/scan_runs/lab_v2_realsetup/divergence_validation"
...
FIRST_JOB_ID="$(tail -n 8 "${PLAN_DIR}/submission-manifest.tsv" | awk -F '\t' 'NR == 1 { print $1 }')"
LAST_JOB_ID="$(tail -n 1 "${PLAN_DIR}/submission-manifest.tsv" | cut -f1)"
ARCHIVE_NAME="lab-v2-divergence-validation-${FIRST_JOB_ID}-${LAST_JOB_ID}.tar.gz"
