...
def main() -> int:
    ...
    for divergence in divergences:
        ...
        surface_rindex = args.surface_rindex or (
            "runner-default"
            if surface_preset in ("polishedbackpainted", "groundbackpainted")
            else "not-applicable"
        )
        grease_absorption_model = args.grease_absorption_model or (
            "not-applicable"
            if args.optical_coupling == "none"
            else "runner-default"
        )
        grease_transmission_csv = args.grease_transmission_csv or (
            "not-applicable"
            if args.optical_coupling == "none"
            else "runner-default"
        )
        lines = [
            ...
            f"# lab_csv={lab_csv}",
            f"# label={args.label}",
            f"# events_per_point={args.events}",
            f"# source_mode={args.source_mode}",
            f"# source_model={args.source_model or 'runner-default'}",
            f"# tank_size={tank_size}",
            f"# points={len(points)}",
            f"# inside_nominal_tile={n_inside}",
            f"# outside_nominal_tile={len(points) - n_inside}",
            f"# beam_z={args.beam_z if args.beam_z is not None else 'template'} {args.grid_unit}",
            f"# beam_divergence_mrad={format_decimal(Decimal(divergence))}",
            f"# beam_sigma={args.beam_sigma if args.beam_sigma is not None else 'point'} {args.grid_unit}",
            f"# surface_preset={surface_preset or 'runner-default'}",
            f"# sipm_size={args.sipm_size if args.sipm_size is not None else 'template'}",
            f"# sipm_face={args.sipm_face}",
            f"# sipm_local_position={args.sipm_local_position}",
            f"# surface_reflectivity_model={args.surface_reflectivity_model or 'runner-default'}",
            f"# surface_reflectivity={args.surface_reflectivity or 'none'}",
            f"# surface_reflectivity_csv={args.surface_reflectivity_csv or 'none'}",
            f"# surface_rindex={surface_rindex}",
            ...
            f"# grease_absorption_model={grease_absorption_model}",
            f"# grease_transmission_csv={grease_transmission_csv}",
        ]
        if args.description:
